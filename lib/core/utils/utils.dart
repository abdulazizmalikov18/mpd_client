...
import 'package:mpd_client/app/constants.dart';
...
class Utils {
  ...
  static String dateFormat(String dateString) {
    ...
    String formattedDate;
    if (isToday) {
      formattedDate = 'Today, ${DateFormat('d MMMM, yyyy').format(dateTime)}';
    } else {
  ...
  static String clockFormat(String dateString) {
    DateTime dateTime = DateTime.parse(dateString);
  ...
  static String imageMediea(List<Media>? media) {
    const defaultImage =
  ...
  static String imageMedieaType(Media? media) {
    const defaultImage =
        "https://avatars.mds.yandex.net/i?id=69aba87029e7a462032b445ab234e3db_l-9095341-images-thumbs&n=13";
    if (media != null) {
      if (media.type == 'video') {
  ...
  static String weekDayFormatObrevation(String dayObrevation) {
    final fullWeekDayString = Constants.weekDays
        .where((element) => element.toLowerCase().startsWith(dayObrevation))
        .first;
