...
class AppoinmentsBloc extends Bloc<AppoinmentsEvent, AppoinmentsState> {
  AppoinmentsBloc(this._profileRepository) : super(const AppoinmentsState()) {
      ...
      emit(
        state.copyWith(
          appoinments: event.getMore
              ? [...state.appoinments, ...result.right.results]
  ...
  Future<void> _onGetAppoinmentsCanceledEvent(
    GetAppoinmentsCanceledEvent event,
    Emitter<AppoinmentsState> emit,
  ) async {
    if (event.getMore) {
