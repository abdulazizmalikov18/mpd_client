...
class ProfessionBloc extends Bloc<ProfessionEvent, ProfessionState> {
  ...
  Future<void> _onGetProfessionsEvent(
      GetProfessionEvent event, Emitter<ProfessionState> emit) async {
    if (state.hasReachedMax) return;

    // Here, Getting professions first time
    if (state is ProfessionInitial) {
      final result =
          await _authRepository.getProfessions(limit: _limit, offset: _offset);
      if (result.isRight) {
        _offset += 15;
        professions.addAll(result.right.results!);
        emit(ProfessionSuccess(
  ...
  Future<void> _onSearchProfessions(
      SearchProfessionsEvent event, Emitter<ProfessionState> emit) async {
  ...
  void errorChecker(Failure failure, Emitter<ProfessionState> emit) {
    if (failure is NetworkFailure) {
      emit(ProfessionFailure(
          professions: state.professions, failure: failure.message));
    } else {
      emit(ProfessionFailure(
          professions: state.professions, failure: 'Server failure'));
    }
  }

  void _onChooseProfession(
          ChooseProfessionEvent event, Emitter<ProfessionState> emit) =>
