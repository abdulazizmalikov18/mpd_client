...
class RegionCategory extends StatefulWidget {
  ...
  const RegionCategory({super.key, required List<Region> regions})
    : _regions = regions;
...
class _ExpansionListState extends State<RegionCategory>
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return widget._regions[index].isParent!
            ? _buildRegionsList(expansionTileKey, widget._regions[index], index)
            : Material(
  ...
  Widget _buildRegionsList(
    ...
    return MyExpTile(
      ...
      initiallyExpanded:
          OpenCloseExpansionInheritedNotifier.of(
            context,
          ).notifier!.selectedTile ==
          index,
      onExpansionChanged: (value) async {
        if (value) {
          Future.delayed(const Duration(milliseconds: 200)).then((value) {
            if (mounted) {
              context.read<DistrictBloc>().add(
                GetDistrictsEvent(parent: region.id),
              );
              OpenCloseExpansionInheritedNotifier.of(
                context,
          ...
          OpenCloseExpansionInheritedNotifier.of(context).notifier!.selectTile =
              -1;
        }
      },
