...
import 'package:mpd_client/features/chat/presentation/views/meet_strat_view.dart';
...
class _WLiveAndMeetingItemState extends State<WLiveAndMeetingItem> {
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      onTap: () {
        // Extract meeting ID from message text
        String meetingId = "";
        if (widget.message?.text != null) {
          // Assuming meeting ID is after #meet# or #live# pattern
          final text = widget.message!.text!;
          final meetIndex = text.indexOf("#meet#");
          final liveIndex = text.indexOf("#live#");

          if (meetIndex != -1) {
            meetingId = text.substring(meetIndex + 6).trim();
          } else if (liveIndex != -1) {
            meetingId = text.substring(liveIndex + 6).trim();
          }
        }

        Navigator.of(context).push(
          MaterialPageRoute(
            builder: (_) => MeetStratView(meetingId: meetingId),
          ),
        );
      },
      child: Container(
