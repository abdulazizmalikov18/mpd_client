...
class InsertCardBloc extends Bloc<InsertCardEvent, InsertCardState> {
  InsertCardBloc(this._profileRepository, this._cardController,
      this._expiryController, this._nameController, this._formkey)
      : super(const InsertCardInitial(null)) {
  ...
  Future<void> _onInsertCard(
      InsertCard event, Emitter<InsertCardState> emit) async {
    if (_formkey.currentState!.validate()) {
      emit(InsertCardLoading(state.card));
      final cardLocalModel = CardLocalModel(
      ...
      if (result.isRight) {
        ...
        emit(InsertCardFailure(
            Utils.errorFormat(result.left.message), state.card));
      }
    }
  }
}
