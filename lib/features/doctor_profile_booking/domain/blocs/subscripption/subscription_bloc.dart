...
class SubscriptionBloc extends Bloc<SubscriptionEvent, SubscriptionState> {
  SubscriptionBloc(this._profileRepository)
      : super(const SubscriptionInitial(false)) {
  ...
  void _onSetSubcribedOrNot(
      SetSubscribedOrNot event, Emitter<SubscriptionState> emit) {
    emit(SubscriptionInitial(
        event.followStatus != null ? event.followStatus! : false));
  }

  Future<void> _onSubscribeToDrEvent(
      SubscribeToDrEvent event, Emitter<SubscriptionState> emit) async {
    ...
    if (result.isRight) {
      ...
      emit(SubscriptionFailure(Utils.errorFormat(result.left.message), false));
    }
  }

  Future<void> _onUnSubscribeToDrEvent(
      UnSubscribeToDrEvent event, Emitter<SubscriptionState> emit) async {
    ...
    if (result.isRight) {
      ...
      emit(SubscriptionFailure(Utils.errorFormat(result.left.message), true,
