...
class BannerBloc extends Bloc<BannerEvent, BannerState> {
  ...
  void _onComentPostChange(
      ComentPostImageChange event, Emitter<BannerState> emit) {
    emit(state.copyWith(
  ...
  void _onInsertComentCount(
      InsertComentToPost event, Emitter<BannerState> emit) {
    for (var post in _banners) {
      if (post.id == event.postId) {
  ...
  void _onBannerIndexChanged(
      BannerIndexChangedEvent event, Emitter<BannerState> emit) {
    emit(state.copyWith(
        currentIndexBanner: event.index, banners: state.banners));
  }

  void _onPostIndexChanged(
      PostIndexChangedEvent event, Emitter<BannerState> emit) {
  ...
  Future<void> _onGetBanners(
      GetBannersEvent event, Emitter<BannerState> emit) async {
    ...
    final result =
        await _homeRepository.getBanners(limit: _limit, offset: _offset);
    if (result.isRight) {
