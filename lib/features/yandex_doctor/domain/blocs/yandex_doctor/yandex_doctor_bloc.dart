...
class YandexDoctorBloc extends Bloc<YandexDoctorEvent, YandexDoctorState> {
  YandexDoctorBloc(this._yandexService)
    ...
    on<AddSpecialistMap>(_onAddSpecialistMap);
  ...
  void _onAddSpecialistMap(
    AddSpecialistMap event,
  ...
  void _onInsertSingleObject(
    InsertSingleObjectEvent event,
    Emitter<YandexDoctorState> emit,
  ) {
    final latitude = event.specialist.lastLocation?.latitude ?? 41.311015;
    final longitude = event.specialist.lastLocation?.longitude ?? 69.279760;
  ...
  void _onClusterizedPlaceMark(
    ClusterPlaceMarkEvent event,
    Emitter<YandexDoctorState> emit,
  ) async {
    final largeMapObject = await _yandexService.getClusterizedPlacemarks(
