...
import 'package:mpd_client/features/home/data/models/yandex_map_object_model.dart';
...
class YandexService extends IYandexService {
    ...
    if (_myCirclePlacemarks.isEmpty) {
      _myCirclePlacemarks = List<CircleMapObject>.generate(2, (i) {
        return CircleMapObject(
          ...
          strokeWidth: 0,
          fillColor: const Color(0xFF3E80FF).withValues(alpha: 0.3),
        );
      });
    }
  ...
  List<PlacemarkMapObject> _getListPlacemarks(
    ...
    for (var i = 0; i < specialists.length; i++) {
      if (specialists[i].lastLocation != null) {
        final Point point = Point(
          latitude: specialists[i].lastLocation?.latitude ?? 41.311015,
          longitude: specialists[i].lastLocation?.longitude ?? 69.279760,
        );
