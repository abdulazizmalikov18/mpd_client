  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(
        ...
        color: widget.isBooked ? background : white,
        gradient: widget.selected ? const LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [gradientBlueOpacity, gradientBlue]) : null,
        border: widget.isBooked
            ? Border.all(width: 1, color: grey)
            : !widget.selected
                ? Border.all(width: 1, color: transparentBorder)
                : Border.all(
                    width: 1,
      ...
      // width: 112.w,
      child: CustomPaint(
        foregroundPainter: widget.isBooked ? const LinePainter(color: grey) : null,
        child: TextButton(
