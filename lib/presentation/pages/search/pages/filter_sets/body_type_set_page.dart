...
class SetBodyTypePage extends StatelessWidget {
  const SetBodyTypePage({super.key});
        ...
        return BlocBuilder<SearchBloc, SearchState>(builder: (context, state) {
          return Scaffold(
            ...
            body: ListView.separated(
                shrinkWrap: true,
                itemCount: state.bodyTypesList?.length ?? 0,
                padding: EdgeInsets.only(bottom: 120.h),
                itemBuilder: (context, index) {
                  int lastIndexWhere = state.filterBodyValue?.lastIndexWhere(
                          (element) =>
                              element.valueId ==
                              state.bodyTypesList?[index].id) ??
                      -1;

                  return CheckboxListItem(
                    disabled:
                        (state.bodyTypesList?[index].countOfCars ?? 0) == 0,
                    colors: colors,
                    fonts: fonts,
                    icons: icons,
                    isSelected: lastIndexWhere != -1,
                    title: state.bodyTypesList?[index].name ?? "",
                    count: (state.bodyTypesList?[index].countOfCars ?? 0)
                        .toString(),
                    onTap: () {
                      List<FilterItemModel> selected =
                          state.filterBodyValue?.toList() ?? [];
                      if (lastIndexWhere != -1) {
                        selected.removeAt(lastIndexWhere);
                      } else {
                        selected.add(FilterItemModel((p0) => p0
                          ..value = state.bodyTypesList?[index].name
                          ..valueId = state.bodyTypesList?[index].id));
                      }
