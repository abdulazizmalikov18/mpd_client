...
class SetTransmissionPage extends StatelessWidget {
  const SetTransmissionPage({super.key});
          ...
          return Scaffold(
            ...
            body: ListView.separated(
                ...
                itemBuilder: (context, index) {
                  final int lastIndexWhere = state.filterTransmissionValue
                  ...
                  return CheckboxListItem(
                    disabled:
                        (state.transmissionList?[index].countOfCars ?? 0) == 0,
                    colors: colors,
                    fonts: fonts,
                    icons: icons,
                    isSelected: lastIndexWhere != -1,
                    title: state.transmissionList?[index].name ?? "",
                    count: (state.transmissionList?[index].countOfCars ?? 0)
                        .toString(),
                    onTap: () {
                      List<FilterItemModel> selected =
                          state.filterTransmissionValue?.toList() ?? [];
                      if (lastIndexWhere != -1) {
                        selected.removeAt(lastIndexWhere);
                      } else {
                        selected.add(FilterItemModel((p0) => p0
                          ..value = state.transmissionList?[index].name
                          ..valueId = state.transmissionList?[index].id));
                      }
